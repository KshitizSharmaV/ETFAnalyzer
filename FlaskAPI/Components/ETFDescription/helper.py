...
def fetchETFsWithSameETFdbCategory(connection=None, ETFdbCategory=None):
    ...
    for item in dataD:
        etfTicker = item['ETFTicker']
        if etfTicker not in tickerlist:
            ETFWithSameETFDBCategory.append({'etfTicker': etfTicker, 'ETFName': item['ETFName'],
                                             'TotalAssetsUnderMgmt': "${:,.3f} M".format(
                                                 item['TotalAssetsUnderMgmt'] / 1000)})
...
def fetchETFsWithSimilarTotAsstUndMgmt(connection=None, totalassetUnderManagement=None):
    ...
    similar_taum_etfs = CollectionName.aggregate([
        {
            '$match': {
            ...
            '$group': {
                '_id': '$ETFTicker',
                'FundHoldingsDate': {
                    '$last': '$FundHoldingsDate'
                },
                'TotalAssetsUnderMgmt': {
                    '$last': '$TotalAssetsUnderMgmt'
                },
                'ETFName': {
                    '$last': '$ETFName'
                }
